...
def get_logger(
    ...
    the top-level ``geovista`` logger.

    The ``geovista`` top-level logger, specified by ``name``, will be configured
    with a :class:`logging.StreamHandler` and a custom :class:`Formatter`,
    ...
    level : int or str, optional
        ...
        for the top-level ``geovista`` logger, otherwise ``INFO`` for child loggers.
